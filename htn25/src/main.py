...
def run(customer_id):
    ...
    try:
        results = Hackathon().crew().kickoff(inputs=inputs)
...
def train(n_iterations=10):
    ...
    try:
        Hackathon().crew().train(n_iterations=n_iterations, filename="trained_model.json")
...
def replay(task_id):
    ...
    try:
        Hackathon().crew().replay(task_id=task_id)
...
def test(n_iterations=5, model_name="deepseek-r1"):
    ...
    try:
        Hackathon().crew().test(n_iterations=n_iterations, openai_model_name=model_name)
